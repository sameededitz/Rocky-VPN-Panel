var App = function () {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    if (scroll) {
                        ...
                        console.warn("Sidebar scroll container `.menu-categories` not found!");
                    }
                }, 50);
            } catch (error) {
                console.error("Error in categoryScroll.scrollCat():", error);
            }
        }
    };
    ...
    var toggleFunction = {
        ...
        themeToggle: function () {
            ...
            if (!togglethemeEl) return; // Prevent errors if the button is missing
            ...
            togglethemeEl.addEventListener('click', function () {
                ...
                // Ensure `layout` exists with default settings
                parseObj.layout = parseObj.layout || {
                    darkMode: false,
                    boxed: true,
                    logo: {
                ...
                // Toggle dark mode
                parseObj.layout.darkMode = !parseObj.layout.darkMode;
                localStorage.setItem("theme", JSON.stringify(parseObj));

                // Apply dark mode class
                document.body.classList.toggle('dark', parseObj.layout.darkMode);

                // Get the correct logo
                var logoSrc = parseObj.layout.darkMode
                    ? parseObj.layout.logo.dark
                    : parseObj.layout.logo.light;

                // Apply logo change
                var navbarLogo = document.querySelector('.navbar-logo');
                if (navbarLogo) {
                    navbarLogo.setAttribute('src', logoSrc);
                }
            });
        }
    }
